...
public class SessionHandlerInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //3 查询用户信息
        PortalUser user = portalUserMapper.selectByPrimaryKey(Long.valueOf(userId));
        if (ObjectUtils.isEmpty(user)){
            response.setStatus(403);
            return false;
        }
        //4. 查看用户token是否失效
        Object redisToken  = redisTemplate.opsForValue().get(DataConstants.UNIFIED_PLATFORM +userId+DataConstants.USER_LOGIN_JWT_TOKEN);
