...
CREATE TABLE IF NOT EXISTS `nmpl_business_route` (
    ...
    `byte_network_id` blob DEFAULT NULL comment '设备入网码(字节存储)',
    PRIMARY KEY (`id`)
...
CREATE TABLE IF NOT EXISTS `nmpl_terminal_data` (
    ...
    `terminal_ip` varchar(64) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '一体机ip',
    `upload_time` datetime(2) DEFAULT NULL COMMENT '上报时间',
    `create_time` datetime(2) DEFAULT CURRENT_TIMESTAMP(2) COMMENT '创建时间',
    `update_time` datetime(2) DEFAULT CURRENT_TIMESTAMP(2) ON UPDATE CURRENT_TIMESTAMP(2) COMMENT '更新时间',
    PRIMARY KEY (`id`)
...
ALTER TABLE `nmpl_local_base_station_info` MODIFY `public_network_port` varchar(128) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL COMMENT '公网端口';
