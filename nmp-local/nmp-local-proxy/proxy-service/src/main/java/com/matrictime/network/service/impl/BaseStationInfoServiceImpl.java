    ...
    public Result addBaseStationInfo(AddBaseStationInfoRequest req) {
        ...
        try {
            ...
            if (infoVo.getIsLocal()){
                ...
                int addBase = baseStationInfoDomainService.insertBaseStationInfo(stationVos);

                // 插入通知表通知base表更新
                updateInfo.setTableName(NMPL_BASE_STATION_INFO);
                int updateBase = nmplUpdateInfoBaseMapper.insertSelective(updateInfo);
                ...
                int addDevice = deviceInfoDomainService.insertDeviceInfo(deviceInfoVos);

                // 插入通知表通知base表更新
                updateInfo.setTableName(NMPL_NMPL_DEVICE_INFO);

                int updateDevice = nmplUpdateInfoBaseMapper.insertSelective(updateInfo);
