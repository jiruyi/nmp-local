...
import com.matrictime.network.schedule.TerminalUserTaskService;
 ...
 * @date 2023/8/23.
 */
@RequestMapping(value = "/scheduleTerminalUser")
@Api(value = "终端用户", tags = "终端用户相关接口")
@RestController
@Slf4j
public class TerminalUserTaskController {

    @Resource
    private TerminalUserTaskService taskService;

    @ApiOperation(value = "终端用户时间间隔修改", notes = "终端用户时间间隔修改")
    @RequestMapping(value = "/update", method = RequestMethod.POST)
    public Result updateScheduleCron(@RequestBody ScheduleCronReq scheduleCronReq) {
        try {
            //参数校验
            checkParam(scheduleCronReq);
            log.info("updateScheduleTerminalUserScheduleCron timer:{}",scheduleCronReq.getConfigValue());
            //更新定时器的间隔时间毫秒值
            taskService.updateTimer(Long.valueOf(scheduleCronReq.getConfigValue())*1000);
            return new Result();
        }catch (Exception e){
            log.error("TerminalUserTaskController updateScheduleCron exception:{}", e);
