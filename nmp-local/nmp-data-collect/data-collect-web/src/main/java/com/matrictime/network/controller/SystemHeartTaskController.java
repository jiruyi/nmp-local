...
import com.matrictime.network.schedule.SystemHeartTaskService;
 ...
 * @date 2023/8/23.
 */
@RequestMapping(value = "/scheduleSystemHeart")
@Api(value = "心跳总数", tags = "心跳总数报相关接口")
@RestController
@Slf4j
public class SystemHeartTaskController {

    @Resource
    private SystemHeartTaskService taskService;

    @ApiOperation(value = "心跳总数间隔修改", notes = "心跳总数时间间隔修改")
    @RequestMapping(value = "/update", method = RequestMethod.POST)
    public Result updateScheduleCron(@RequestBody ScheduleCronReq scheduleCronReq) {
        try {
            //参数校验
            checkParam(scheduleCronReq);
            log.info("updateSystemHeartScheduleCron timer:{}",scheduleCronReq.getConfigValue());
            //更新定时器的间隔时间毫秒值
            taskService.updateTimer(Long.valueOf(scheduleCronReq.getConfigValue())*1000);
            return new Result();
        }catch (Exception e){
            log.error("SystemHeartTaskController updateScheduleCron exception:{}", e);
