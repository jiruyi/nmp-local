...
import com.matrictime.network.schedule.DeviceTaskService;
 ...
 * @date 2023/8/23.
 */
@RequestMapping(value = "/scheduleDevice")
...
public class DeviceTaskController {
    ...
    public Result updateScheduleCron(@RequestBody ScheduleCronReq scheduleCronReq) {
        try {
            ...
            log.info("updateScheduleDeviceCron timer:{}",scheduleCronReq.getConfigValue());
            //更新定时器的间隔时间毫秒值
            taskService.updateTimer(Long.valueOf(scheduleCronReq.getConfigValue())*1000);
            return new Result();
        }catch (Exception e){
            log.error("DeviceTaskController updateScheduleCron exception:{}", e);
