...
import com.matrictime.network.schedule.CompanyHeartbeatTaskService;
 ...
 * @date 2023/8/23.
 */
@RequestMapping(value = "/scheduleCompanyHeartbeat")
@Api(value = "小区心跳", tags = "小区心跳相关接口")
@RestController
@Slf4j
public class CompanyHeartbeatTaskController {

    @Resource
    private CompanyHeartbeatTaskService taskService;
    ...
    public Result updateScheduleCron(@RequestBody ScheduleCronReq scheduleCronReq) {
        try {
            ...
            log.info("updateCompanyHeartbeatScheduleCron timer:{}",scheduleCronReq.getConfigValue());
            //更新定时器的间隔时间毫秒值
            taskService.updateTimer(Long.valueOf(scheduleCronReq.getConfigValue())*1000);
            return new Result();
        }catch (Exception e){
            log.error("CompanyHeartbeatTaskController updateScheduleCron exception:{}", e);
