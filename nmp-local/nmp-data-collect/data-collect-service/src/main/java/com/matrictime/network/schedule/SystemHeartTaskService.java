...
@Slf4j
@Component
@BusinessType(businessType = BusinessTypeEnum.BUSINESS_HEART)
        ...
        try {
            ...
            if(channelFuture.isDone()){
                if (!channelFuture.isSuccess()){
                    log.info("systemHeartTaskService  nettyClient.sendMsg error :{}", channelFuture.cause());
                    return;
                }
                if(channelFuture.isSuccess()){
            ...
            log.error("SystemHeartTaskService configureTasks exception:{}", e);
            return;
        }
