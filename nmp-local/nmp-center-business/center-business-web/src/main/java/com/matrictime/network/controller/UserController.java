...
//    @SystemLog(opermodul = "用户管理模块",operDesc = "用户退出下系统",operType = "登出")
    ...
    public Result<LoginResponse> logout(@RequestBody LoginRequest loginRequest){
        try {
            ...
            redisTemplate.delete(DataConstants.ACCUSATION_CENTER+loginRequest.getUserId()+ DataConstants.USER_LOGIN_JWT_TOKEN);
            //删除用户登录状态
            redisTemplate.delete(DataConstants.ACCUSATION_CENTER+loginRequest.getUserId()+DataConstants.USER_LOGIN_STATUS);
